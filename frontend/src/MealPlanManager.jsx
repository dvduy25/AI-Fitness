...
import { useNavigate } from 'react-router-dom'; // BỔ SUNG: Dùng để chuyển hướng sang trang Premium
import { 
...
import PremiumRequireModal from './PremiumRequireModal'; // BỔ SUNG: Import bảng thông báo
...
export default function MealPlanManager() {
  ...
  // ==========================================
  // BỔ SUNG: STATE CHO TÍNH NĂNG PREMIUM / QUẢNG CÁO
  // ==========================================
  ...
  // ==========================================
  // BỔ SUNG: HÀM KIỂM TRA QUYỀN TRUY CẬP AI
  // ==========================================
  const checkAiAccess = () => {
  ...
  // ==========================================
  // BỔ SUNG: XỬ LÝ NÚT XEM QUẢNG CÁO ẢO
  // ==========================================
  const handleWatchAd = async () => {
    ...
    try {
      ...
      setShowPremiumModal(false); // Tắt bảng chặn
    } catch (error) {
  ...
  // ==========================================
  // CHỈNH SỬA: CHẶN TRƯỚC KHI TẠO LỊCH ĂN
  // ==========================================
  const handleGeneratePlan = async () => {
    // 1. Chặn lại nếu hết quyền (Hết VIP & Hết Vé)
    if (!checkAiAccess()) {
  ...
  // ==========================================
  // BỔ SUNG: HÀM CHẶN TRƯỚC KHI BẬT BẢNG ĐÁNH GIÁ AI
  // ==========================================
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800 shadow-lg relative overflow-hidden">
              ...
              {/* NÚT AI TẠO LỊCH ĐÃ ĐƯỢC BỌC TRONG HÀM KIỂM TRA QUYỀN */}
              <button 
              ...
              >
                {isGenerating ? <><Loader2 className="w-5 h-5 animate-spin" /> Đang phân tích...</> : <><Sparkles className="w-5 h-5" /> TẠO LỊCH ĂN</>}
              </button>
            </div>
          </div>
            ...
            <div className="bg-gray-900 min-h-[500px] p-4 md:p-6 rounded-2xl border border-gray-800 shadow-lg">
              ...
              {!isLoadingPlan && !isGenerating && !generatedPlan && (
                <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center border-2 border-dashed border-gray-800 rounded-xl bg-gray-900/50">
                  ...
                  <button onClick={() => setShowAddMealModal(true)} className="mt-4 px-6 py-2.5 bg-gray-800 hover:bg-gray-700 text-white font-medium rounded-lg">
                    + Tạo lịch thủ công
                  </button>
                </div>
              )}
